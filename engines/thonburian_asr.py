...
def _get_pipeline():
    """Lazy-load the Thonburian Whisper pipeline with OpenVINO backend."""
    global _pipeline
    ...
    if os.path.isdir(export_dir) and os.path.isfile(os.path.join(export_dir, "openvino_encoder_model.xml")):
        ...
        logger.info("Exporting Thonburian to OpenVINO IR (first run, may take several minutes)...")
        model = OVModelForSpeechSeq2Seq.from_pretrained(
            MODEL_ID,
            export=True,
            device=device,
...
def transcribe_thonburian(audio_path: str, language: str = "thai", num_speakers: int = 0, min_speakers: int = 0, max_speakers: int = 0) -> str:
